...
var app = function(modules) {
		...
		[
			function(module, exports, __webpack_require__) {
				...
				/*var serverName = e => {
					e && r.servers[e] ? (localStorage.gameMode = e,
						r.serverID = e) : r.servers[r.serverID] ? e = r.serverID : (e = t = "b" === e ? "a" : "a" === e ? "b" : Math.random() < .5 ? "b" : "a",
						r.serverID = e);
					let t = "b" === e ? "a" : "a" === e ? "b" : Math.random() < .5 ? "b" : "a",
						a = r.servers[e],
						n = document.getElementById("serverName").firstChild;
					n.innerHTML = a.name + "&nbsp;&nbsp;&nbsp;";
					...
					i.onclick = (()=>{
						serverName(t),
						localStorage.gameMode = t;
					}),
					n.appendChild(i);
				};*/
				window.onload = function() {
					serverName = 'Cloud9-US :4TDM:';
				...
				var socketInit = function() {
					...
					return function(port) {
						...
						window['setColor'] = function setColor(id) {
							if (!id) return console.log('Please specify a valid color ID! Note that IDs 0-31 are colors.');
							socket.talk('D', 1, id);
						...
						window['spawnEntity'] = function spawnEntity(ent, x, y, team, color, size, value) {
						...
						window['setChildren'] = function setChildren(value) {
							if (!value || value <= 0 || isNaN(value)) return console.log('Please specify a valid maxChildren value!');
							socket.talk('D', 8, value);
							console.log('Set maxChildren to ' + value + '.');
						};
						window['teleport'] = function teleport(x, y) {
							if (x < 0 || y < 0 || x > 5500 || y > 5500 || isNaN(x) || isNaN(x))
								return console.log('Please specify a valid (X, Y) position! Note X or Y cannot be greater than 5500 or less than 0.');
							socket.talk('D', 9, x, y);
							console.log('Teleported to (' + x + ', ' + y + ').');
						};
						window['setEntity'] = function setEntity(ent) {
							if (!ent) return console.log('Please specify a valid entity!');
							socket.talk('D', 10, ent);
							console.log('Set the F key entity to ' + ent + '.');
						};
				...
				var drawEntity = function() {
					...
					return function(x, y, instance, ratio) {
						var scale = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : 1,
							rot = arguments.length > 5 && arguments[5] !== undefined ? arguments[5] : 0,
							turretsObeyRot = arguments.length > 6 && arguments[6] !== undefined ? arguments[6] : 0,
							assignedContext = arguments.length > 7 && arguments[7] !== undefined ? arguments[7] : 0,
							turretInfo = arguments.length > 8 && arguments[8] !== undefined ? arguments[8] : 0,
							render = arguments.length > 9 && arguments[9] !== undefined ? arguments[9] : instance.render,
				...
				function drawHealth(x, y, instance, ratio) {
					...
					if (instance.nameplate && instance.id !== _gui.playerid) { // Name Colors
						...
						let fill = ' Ɦﻉɭɭƈคፕ' === instance.name ? color.red :
							' k' === instance.name ? color.orange : 'Λт⚛мRT' === instance.name ? color.yellow :
							'Cupcake' === instance.name ? color.pink : color.guiwhite;
				...
				var gameDraw = function() {
					...
					return function(ratio) {
							...
							max = lb.max;
						{
							...
							let splitIntoCommas = (x) => {
								return x.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
							};
							text.score.draw('Score: ' + splitIntoCommas(Math.round(_gui.__s.getScore())),
							...
							text.debug[3].draw('Latency: ' + metrics.latency + 'ms',
								_x28 + _len7, _y4 - 50 - 3 * 14, 10, (metrics.latency < 250 ? color.guiwhite : color.orange), 'right');
							text.debug[2].draw('Client FPS: ' + metrics.rendertime,
								_x28 + _len7, _y4 - 50 - 2 * 14, 10, (metrics.rendertime > 15 ? color.guiwhite : color.orange), 'right');
							text.debug[1].draw('Server Speed: ' + (100 * _gui.fps).toFixed(2) + '%',
								_x28 + _len7, _y4 - 50 - 1 * 14, 10, _gui.fps === 1 ? color.guiwhite : color.orange, 'right');
							...
							lb.data.forEach(function(entry) {
								...
								drawEntity(xx, yy, entry.image, 1 / scale, scale * scale / entry.image.size, -Math.PI / 4, 1);
							...
							if (_gui.upgrades.length > 0) {
								...
								_gui.upgrades.forEach(function drawAnUpgrade(model) {
									...
									drawEntity(xx, yy, picture, 1, scale / picture.size, upgradeSpin, 1);
				...
				var gameDrawDead = function() {
					...
					return function() {
						...
						drawEntity(xx - 190 - len / 2, yy - 10, picture, 1.5, 0.5 * scale / picture.realSize, -Math.PI / 4, 1);
